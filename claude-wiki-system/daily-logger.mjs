...
function getMainLogActivity() {
  ...
  if (errors.length > 0) {
    activity.push(`${errors.length} error(s) logged (see main.log for details)`);
  }
  ...
  for (const session of sessions) {
    ...
    for (const file of files) {
      ...
      if (stat.mtimeMs >= todayMidnight) {
        // Read first line to get the file content hint
...
function getObservations() {
  ...
  for (const proj of fs.readdirSync(hDir)) {
    ...
    if (stat.mtimeMs < todayMidnight) continue;
...
function updateIndex() {
  ...
  const newLine = `- [Daily log ${todayStr}](log.md) — end-of-day activity summary\n`;
